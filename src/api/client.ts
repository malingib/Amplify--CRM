...
export async function api<T = any>(path: string, options: ApiOptions = {}): Promise<T> {
  ...
  const execute = async (): Promise<T> => {
    ...
    try {
      ...
      if (res.status === 401 && !path.startsWith('/auth/login') && !path.startsWith('/auth/register') && !path.startsWith('/auth/google')) {
        clearToken();
...
export const leadsApi = {
  ...
  convertToClient: (leadId: string) => api(`/lead-lifecycle/${leadId}/convert`, { method: 'POST' }),
};
