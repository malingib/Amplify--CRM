...
// Phase 2: CRM command-center data, scoped for the current sales team.
router.get('/command-center', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({
      ...
      stages: Object.keys(stageWeights).map(stage => ({
        stage,
        count: leads.filter(l => l.stage === stage).length,
        value: leads.filter(l => l.stage === stage).reduce((sum, l) => sum + l.value, 0),
        weightedValue: leads.filter(l => l.stage === stage).reduce((sum, l) => sum + l.value * (l.probability / 100), 0),
      })),
      leads,
      tasks,
    });
  } catch (error) {
    console.error('CRM command center error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// Phase 2: duplicate detection before creating or importing a lead.
router.get('/duplicates', authenticate, authorize(...SALES_ROLES), async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (company && name) clauses.push({ company, name });
...
// Phase 2/3: controlled bulk operations with one transaction.
router.post('/bulk/leads', authenticate, authorize(...SALES_ROLES), async (req: AuthRequest, res: Response) => {
  try {
    const { ids, stage, probability, ownerId } = req.body as {
      ids?: string[]; stage?: string; probability?: number; ownerId?: string | null;
    };
    if (!Array.isArray(ids) || ids.length === 0 || ids.length > 100) {
      return res.status(400).json({ error: 'ids must contain between 1 and 100 leads' });
    }
    if (!stage && probability === undefined && ownerId === undefined) {
      return res.status(400).json({ error: 'No bulk changes supplied' });
    }
    const data: any = {};
    if (stage) {
    ...
    if (probability !== undefined) {
      if (!Number.isInteger(probability) || probability < 0 || probability > 100) return res.status(400).json({ error: 'Probability must be 0-100' });
...
// Phase 3: forecast by stage and owner, using the stored probability as the primary forecast signal.
router.get('/forecast', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const byStage = Object.keys(stageWeights).filter(s => s !== 'CLOSED' && s !== 'LOST').map(stage => {
      const items = open.filter(l => l.stage === stage);
      return { stage, count: items.length, value: items.reduce((s, l) => s + l.value, 0), weightedValue: items.reduce((s, l) => s + l.value * l.probability / 100, 0) };
    });
    const byOwner = Array.from(new Map(open.map(l => [l.ownerId || 'unassigned', l.owner])).entries()).map(([ownerId, owner]) => {
      const items = open.filter(l => (l.ownerId || 'unassigned') === ownerId);
      return { ownerId, ownerName: owner?.name || 'Unassigned', value: items.reduce((s, l) => s + l.value, 0), weightedValue: items.reduce((s, l) => s + l.value * l.probability / 100, 0), count: items.length };
    });
...
// Phase 4: unified communication history. This records outbound/inbound activity without pretending delivery occurred.
router.post('/communications/log', authenticate, authorize(...SALES_ROLES), async (req: AuthRequest, res: Response) => {
  try {
    const { leadId, type, direction, content, status = 'logged' } = req.body as {
      leadId: string; type?: string; direction?: 'inbound' | 'outbound'; content: string; status?: string;
    };
    if (!leadId || !content?.trim()) return res.status(400).json({ error: 'leadId and content are required' });
    const lead = await prisma.lead.findUnique({ where: { id: leadId } });
    if (!lead) return res.status(404).json({ error: 'Lead not found' });

    const channel = (type || 'Note').trim();
    const prefix = `[${channel}${direction ? ` • ${direction}` : ''} • ${status}]`;
...
router.post('/leads/:id/move', authenticate, authorize(...SALES_ROLES), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const lead = await prisma.lead.update({ where: { id: current.id }, data: { stage, probability: nextProbability } });
    ...
    console.error('Lead move error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
